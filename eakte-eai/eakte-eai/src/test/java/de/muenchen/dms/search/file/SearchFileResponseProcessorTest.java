...
class SearchFileResponseProcessorTest {

  private static final int NO_ERROR_CODE = 0;

  private SearchFileResponseProcessor processor;

  @BeforeEach
  void setUp() {
    processor = new SearchFileResponseProcessor();
  }
  ...
  private Exchange erzeugeExchangeObjekt(SearchFileGIResponse response) {
  ...
  private SearchFileGIResponse erzeugeDmsAntwortOhneFehler() {
    SearchFileGIResponse response = new SearchFileGIResponse();
  ...
  private void pruefeEAIAntwort(Exchange exchange) throws IOException {
    ...
    SearchFileResponseDTO response =
        mapper.createParser(body).readValueAs(SearchFileResponseDTO.class);
