...
class SearchObjNameResponseProcessorTest {
  private static final int NO_ERROR_CODE = 0;

  private static final String PREFIX_DOCUMENT = "document";

  private SearchObjNameResponseProcessor processor;

  @BeforeEach
  void setUp() {
    processor = new SearchObjNameResponseProcessor();
  }
  ...
  private void pruefeEAIAntwort(Exchange exchange) throws IOException {
    ...
    SearchObjNameAntwortDTO antwort =
        mapper.createParser(body).readValueAs(SearchObjNameAntwortDTO.class);
  ...
  private Exchange erzeugeExchangeObjekt(SearchObjNameGIResponse response) {
  ...
  private SearchObjNameGIResponse erzeugeDmsAntwortOhneFehler() {
    ObjectFactory objectFactory = new ObjectFactory();

    SearchObjNameGIResponse response = new SearchObjNameGIResponse();
