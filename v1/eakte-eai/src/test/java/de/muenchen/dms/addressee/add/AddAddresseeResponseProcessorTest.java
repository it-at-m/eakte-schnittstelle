...
class AddAddresseeResponseProcessorTest {
  private static final String ERROR_MESSAGE = "Fehler aufgetreten";
  private static final int ERROR_CODE = 200;
  private static final int NO_ERROR_CODE = 0;

  private AddAddresseeResponseProcessor processor;

  @BeforeEach
  void setUp() {
    processor = new AddAddresseeResponseProcessor();
  }
  ...
  private Exchange erzeugeExchangeObjekt(AddAddresseeResponse response) {
  ...
  private AddAddresseeResponse erzeugeDmsAntwortOhneFehler() {
    AddAddresseeResponse response = new AddAddresseeResponse();
    response.setStatus(NO_ERROR_CODE);
    response.setErrormessage(null);
    return response;
  }

  private AddAddresseeResponse erzeugeDmsAntwortMitFehler() {
    AddAddresseeResponse response = new AddAddresseeResponse();
