...
import de.muenchen.dms.schriftstueck.ausgang.lesen.ReadOutgoingAntwortDTO;
import de.muenchen.dms.schriftstueck.ausgang.lesen.ReadOutgoingResponseProcessor;
...
class ReadOutgoingResponseProcessorTest {
  ...
  private ReadOutgoingResponseProcessor processor;

  @BeforeEach
  public void setUp() {
    processor = new ReadOutgoingResponseProcessor();
  }
  ...
  private Exchange erzeugeExchangeObjekt(ReadOutgoingGIResponse response) {
  ...
  private void pruefeEAIAntwort(Exchange exchange) throws IOException {
    ...
    ReadOutgoingAntwortDTO antwort =
        mapper.createParser(body).readValueAs(ReadOutgoingAntwortDTO.class);
  ...
  private ReadOutgoingGIResponse erzeugeDmsAntwortOhneFehler() {
    ReadOutgoingGIResponse response = new ReadOutgoingGIResponse();
    response.setStatus(NO_ERROR_CODE);
    response.setErrormessage(null);
    response.setObjname(OBJECT_NAME);
    response.setAccdef(ACCDEF);

    ArrayOfLHMBAI151700GIMetadataType listmetadataType;
    listmetadataType =
    ...
    response.setGimetadatatype(listmetadataType);

    return response;
  }

  private static List<MetadataReferenz> erzeugeBeispielMetadataReferenz() {
