...
@Validated
@Controller
@Tag(name = "/cancelObjects", description = "the /cancelObjects API")
public interface CancelObjectsApi {

    default Optional<NativeWebRequest> getRequest() {
        return Optional.empty();
    }

    String PATH_CANCEL_OBJECT = "/cancelObjects/{objaddress}";
    /**
     * PUT /cancelObjects/{objaddress} : Objekt stornieren (Vorgang, Dokument, Schriftstück)
     * Stornieren wird verwendet, wenn fälschlicherweise ein Schriftgutobjekt erstellt worden ist.&lt;br&gt; Nach dem Stornieren kann das stornierte Objekt über die Schnittstelle nicht mehr ausgelesen werden.&lt;br&gt; Das Aufheben von stornierten Objekten ist nur in der GUI der eGov-Suite möglich, nicht über die Schnittstelle.  Der Status [FSCFOLIO@1.1001:objdocstate] des Objekts wird auf „Storniert“ gesetzt.  Zudem erfolgt gemäß dem Berechtigungskonzept ein Access Control List (ACL) Übergang.&lt;br&gt; Welcher ACL-Übergang beim Stornieren erfolgt, ist im Berechtigungskonzept definiert und  ist abhängig von der hinterlegten ACL des zu stornierenden Objekts.&lt;br&gt; Beispiel: Eine Akte hat die ACL für Schriftgutobjekte des Eigentümers.  Im Rahmen der Stornierung erfolgt der ACL-Übergang und  die Akte erhält die ACL für abgeschlossene Schriftgutobjekte des Eigentümers.  Bei einer Stornierung werden auch alle untergeordneten Schriftgutobjekte storniert. 
     *
     ...
     */
    @Operation(
        operationId = "cancelObject",
        summary = "Objekt stornieren (Vorgang, Dokument, Schriftstück)",
        description = "Stornieren wird verwendet, wenn fälschlicherweise ein Schriftgutobjekt erstellt worden ist.<br> Nach dem Stornieren kann das stornierte Objekt über die Schnittstelle nicht mehr ausgelesen werden.<br> Das Aufheben von stornierten Objekten ist nur in der GUI der eGov-Suite möglich, nicht über die Schnittstelle.  Der Status [FSCFOLIO@1.1001:objdocstate] des Objekts wird auf „Storniert“ gesetzt.  Zudem erfolgt gemäß dem Berechtigungskonzept ein Access Control List (ACL) Übergang.<br> Welcher ACL-Übergang beim Stornieren erfolgt, ist im Berechtigungskonzept definiert und  ist abhängig von der hinterlegten ACL des zu stornierenden Objekts.<br> Beispiel: Eine Akte hat die ACL für Schriftgutobjekte des Eigentümers.  Im Rahmen der Stornierung erfolgt der ACL-Übergang und  die Akte erhält die ACL für abgeschlossene Schriftgutobjekte des Eigentümers.  Bei einer Stornierung werden auch alle untergeordneten Schriftgutobjekte storniert. ",
        tags = { "/cancelObjects" },
        responses = {
    ...
    @RequestMapping(
        ...
        value = CancelObjectsApi.PATH_CANCEL_OBJECT,
        produces = { "application/json" }
    )
    default ResponseEntity<Void> cancelObject(
