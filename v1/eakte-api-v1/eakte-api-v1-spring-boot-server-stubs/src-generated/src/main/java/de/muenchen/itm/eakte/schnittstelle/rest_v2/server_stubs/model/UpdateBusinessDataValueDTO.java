...
public class UpdateBusinessDataValueDTO {
  ...
  /**
   * Referenz des Fachdatums\\ Hier kann die einfache Referenz des Komponentenobjektes angeführt werden (\"FD_Position\" anstelle \"COOSYSTEM@1.1:FD_Position\".\\ Die Komponentenobjekte können sowohl von Fabasoft in der CFGMUENCHEN-Komponente  sowie von Domänenadministratoren in einer lokalen Softwarekomponenten erzeugt werden.  Wenn das Fachdaten-Komponentenobjekt gefunden wird,  wird diese unabhängig von der darüberliegenden Konfiguration gesetzt  d.h. eine zusätzliche Prüfung, ob diese in den darüberliegenden Aktenplaneinträgen vorgesehen ist,  kann nicht erfolgen. Dafür gibt es die neue Funktion /userFormsData, mit der Verfahrensdaten abgespeichert werden.\\ Für vorhandene Schnittstellen werden ab MUCS26 Fachdaten automatisch in den Verfahrensdaten abgespeichert. 
   * @return reference
   */
  @NotNull 
  @Schema(name = "reference", example = "data_dognumber", description = "Referenz des Fachdatums\\ Hier kann die einfache Referenz des Komponentenobjektes angeführt werden (\"FD_Position\" anstelle \"COOSYSTEM@1.1:FD_Position\".\\ Die Komponentenobjekte können sowohl von Fabasoft in der CFGMUENCHEN-Komponente  sowie von Domänenadministratoren in einer lokalen Softwarekomponenten erzeugt werden.  Wenn das Fachdaten-Komponentenobjekt gefunden wird,  wird diese unabhängig von der darüberliegenden Konfiguration gesetzt  d.h. eine zusätzliche Prüfung, ob diese in den darüberliegenden Aktenplaneinträgen vorgesehen ist,  kann nicht erfolgen. Dafür gibt es die neue Funktion /userFormsData, mit der Verfahrensdaten abgespeichert werden.\\ Für vorhandene Schnittstellen werden ab MUCS26 Fachdaten automatisch in den Verfahrensdaten abgespeichert. ", requiredMode = Schema.RequiredMode.REQUIRED)
