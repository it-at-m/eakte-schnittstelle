...
public class UpdateAccessDefinitionObjectAnfrageDTO {
   ...
   */
  @NotNull 
  @Schema(name = "objaccdef", example = "Eigener Mandant", description = "Folgende Zugriffsdefinitionen sind möglich:  * \"Eigene Organisationseinheit und Linienorganisation\" * \"Eigener Mandant\" * \"Eigene Organisationseinheit und Vorgesetzte\" * \"Eigene Organisationseinheit und Vorgesetzte (ohne Zentralregistratur)\" * \"Eigene Organisationseinheit\" * \"Persönlich und Vorgesetzte\" * \"Persönlich\" (für Verschlusssachen) * \"Aktengebunden\" (nur für untergeordnete Vorgänge einer Akte)  Die Zugriffsdefinition wird jeweils auf die darunterliegenden Objekte vererbt,  d.h.  * wenn ein Wert für den Aktenplaneintrag eingetragen wird (empfohlen),  dann gilt dieser automatisch für alle darunter liegenden Akten,  * entsprechend für alle Vorgänge einer Akte,  wenn die Zugriffsdefinition bei der Akte eingetragen wurde und  * entsprechend bei allen Dokumenten eines Vorgangs,  wenn die Zugriffsdefinition bei dem Vorgang eingetragen wurde.  Wenn kein Wert übergeben wird,  wird die auf dem jeweils höheren Objekt hinterlegte Zugriffsdefinition initialisiert. Wurde auf dem Aktenplaneintrag keine Zugriffsdefinition festgelegt,  wird die im System eingestellte Definition verwendet - \"Eigener Mandant\".  Wenn Vorgänge die Zugriffsdefinition und zusätzlich berechtigte Organisationseinheiten  aus der übergeordneten Akte übernehmen sollen,  muss \"Aktengebunden\" für Vorgänge bei der Akte angegeben werden (procaccdef). Wenn eine Akte über die SST angelegt wird und  am Aktenplaneintrag zusätzlich berechtigte OE's eingetragen wurden  (im Reiter \"Sicherheitseinstellungen\" des Aktenplaneintrags),  dann wird die Zugriffsdefinition \"Aktengebunden\" automatisch für Vorgänge gesetzt. ", requiredMode = Schema.RequiredMode.REQUIRED)
  ...
  /**
   * Rechtevererbung<br> Mögliche Werte: null,0,1,2  0 oder leer (null): Die Rechtevererbung aus dem Standardprodukt wird verwendet.<br> Der neue Wert wird rekursiv gesetzt in Abhängigkeit von den jeweiligen aktuell gesetzten Werten der untergeordneten Objekte.  1: Änderung der Zugriffsdefinition auf dem Objekt und rekursive Vererbung  auf alle untergeordneten Objekte unabhängig von den jeweiligen aktuell gesetzten Werten der untergeordneten Objekte.  2: Änderung der Zugriffsdefinition nur auf dem Objekt  Hinweis:\\  Beim übergeben des Werts 1 auf einer Sachakte vererbt sich die Zugriffsdefinition auf alle in der Akte enthaltenen Vorgänge und wieder auf alle Dokumente,  die in den Vorgängen enthalten sind.<br> Bei einer großen Anzahl von untergeordneten Objekten kann es zu Abbrüchen  aufgrund Objektsperren oder durch Timeouts kommen. 
   * @return authinheritance
   */
  
  @Schema(name = "authinheritance", example = "1", description = "Rechtevererbung<br> Mögliche Werte: null,0,1,2  0 oder leer (null): Die Rechtevererbung aus dem Standardprodukt wird verwendet.<br> Der neue Wert wird rekursiv gesetzt in Abhängigkeit von den jeweiligen aktuell gesetzten Werten der untergeordneten Objekte.  1: Änderung der Zugriffsdefinition auf dem Objekt und rekursive Vererbung  auf alle untergeordneten Objekte unabhängig von den jeweiligen aktuell gesetzten Werten der untergeordneten Objekte.  2: Änderung der Zugriffsdefinition nur auf dem Objekt  Hinweis:\\  Beim übergeben des Werts 1 auf einer Sachakte vererbt sich die Zugriffsdefinition auf alle in der Akte enthaltenen Vorgänge und wieder auf alle Dokumente,  die in den Vorgängen enthalten sind.<br> Bei einer großen Anzahl von untergeordneten Objekten kann es zu Abbrüchen  aufgrund Objektsperren oder durch Timeouts kommen. ", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
